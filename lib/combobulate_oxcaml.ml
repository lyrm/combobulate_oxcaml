...
let squares_of_evens n =
  [ x * x for x = 1 to n when x mod 2 = 0 ]
...
let even_iarray : int iarray =
  [: x for x = 1 to 20 when x mod 2 = 0 :]
...
let%template [@kind k = (value, value & value)] id_any
  : ('a : k). 'a -> 'a
  =
  fun x -> x
...
let origin : (x:float * y:float * z:float) =
  (~x:0.0, ~y:0.0, ~z:0.0)
